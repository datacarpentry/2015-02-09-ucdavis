...
`subject_id`. You want your object names to be *explicit* so they help you
remember what they represent.

You don't want variable names to be too long, but they can be somewhat long
RStudio, and many other programs have a feature called **tab completion**.
Start typing the name of a function, or variable and hit TAB, and RStudio will
complete the name if the object is stored in memory.  If it's ambiguous, it
will give you options

<!-- Demonstrate this with `weight_kg`, `help()`, etc. -->

Tab completion lets you err on the size of more transparent names.

Variable names cannot start with a number (`2x` is not valid but `x2` is). R is case sensitive
